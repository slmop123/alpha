...
async function startServer() {
  ...
  // Configure JSON parser with generous payload limits for base64 image transfers
  ...
  app.post("/api/scan", async (req, res) => {
    try {
      ...
      // 2. Initialize Gemini SDK using the correct client configuration
      ...
      // 3. Define systemic Arabic instructions
      ...
      // 4. Transform safe base64-encoded string into the typed inline part
      const imagePart = {
        inlineData: {
      ...
      // 5. Fire query via resilient content generation with retry & fallback mechanics
      ...
      console.error("Gemini server-side integration fail:", error);
